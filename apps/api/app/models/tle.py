...
class TLE(Base):
    ...
    line1: Mapped[str] = mapped_column(String(80), nullable=False)  # usually 69 chars, allow up to ~80
    ...
    eccentricity: Mapped[float]      = mapped_column(Numeric(9, 7), nullable=False)  # e (decimal, not 7-digit integer)
    ...
    mean_motion_rev_per_day: Mapped[float] = mapped_column(Numeric(12, 8), nullable=False)  # n
    ...
    fetched_at: Mapped            = mapped_column(DateTime(timezone=True), server_default=func.now())
